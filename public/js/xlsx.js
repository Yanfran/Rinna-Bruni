...
function make_xlsx_lib(XLSX) {
    ...
    function utf16leread(data) {
        var o = [];
        for (var i = 0; i < (data.length >> 1); ++i) o[i] = String.xfromCharCode(data.charCodeAt(2 * i) + (data.charCodeAt(2 * i + 1) << 8));
        return o.join("");
    }
    
    function utf16beread(data) {
        var o = [];
        for (var i = 0; i < (data.length >> 1); ++i) o[i] = String.xfromCharCode(data.charCodeAt(2 * i + 1) + (data.charCodeAt(2 * i) << 8));
        return o.join("");
    }
    ...
    if (typeof cptable !== 'undefined') {
        ...
        _getchar = function _gc2(x) {
            if (current_codepage === 1200) return String.xfromCharCode(x);
        ...
        return {
            ...
            decode: function b64_decode(input) {
                ...
                for (var i = 0; i < input.length;) {
                    ...
                    c1 = (e1 << 2) | (e2 >> 4);
                    o += String.xfromCharCode(c1);
                    
                    e3 = map.indexOf(input.charAt(i++));
                    c2 = ((e2 & 15) << 4) | (e3 >> 2);
                    if (e3 !== 64) {
                        o += String.xfromCharCode(c2);
                    }
                    
                    e4 = map.indexOf(input.charAt(i++));
                    c3 = ((e3 & 3) << 6) | e4;
                    if (e4 !== 64) {
                        o += String.xfromCharCode(c3);
                    }
                }
                return o;
            }
            ...
            while (i < fmt.length) {
                switch ((c = fmt.charAt(i))) {
                    case 'G': /* General */
                        ...
                        break;
                    case '"': /* Literal text */
                        for (o = ""; (cc = fmt.charCodeAt(++i)) !== 34 && i < fmt.length;) o += String.xfromCharCode(cc);
...
//var MSCSZ = 4096; /* Mini Stream Cutoff Size */
        ...
        function a2s(o) {
            ...
            for (var i = 0; i < o.length; ++i) out[i] = String.xfromCharCode(o[i]);
            return out.join("");
        }
    ...
    function cc2str(arr) {
        var o = "";
        for (var i = 0; i != arr.length; ++i) o += String.xfromCharCode(arr[i]);
        return o;
    }
...
// TODO: CP remap (need to read file version to determine OS)
    var unescapexml = (function () {
        ...
        return function unescapexml(text) {
            ...
            if (i == -1) return s.replace(encregex, function ($$, $1) {
                return encodings[$$] || String.xfromCharCode(parseInt($1, $$.indexOf("x") > -1 ? 16 : 10)) || $$;
            }).replace(coderegex, function (m, c) {
                return String.xfromCharCode(parseInt(c, 16));
            });
            var j = s.indexOf("]]>");
    ...
    var xlml_fixstr = (function () {
        ...
        function entrepl($$, $1) {
            return String.xfromCharCode(parseInt($1, 10));
        }
    ...
    var utf8read = function utf8reada(orig) {
        ...
        while (i < orig.length) {
            ...
            if (c > 191 && c < 224) {
                f = ((c & 31) << 6);
                f |= (d & 63);
                out += String.xfromCharCode(f);
                continue;
            }
            e = orig.charCodeAt(i++);
            if (c < 240) {
                out += String.xfromCharCode(((c & 15) << 12) | ((d & 63) << 6) | (e & 63));
                continue;
            }
            f = orig.charCodeAt(i++);
            w = (((c & 7) << 18) | ((d & 63) << 12) | ((e & 63) << 6) | (f & 63)) - 65536;
            out += String.xfromCharCode(0xD800 + ((w >>> 10) & 1023));
            out += String.xfromCharCode(0xDC00 + (w & 1023));
        }
        return out;
    };
    ...
    var utf8write = function (orig) {
        ...
        while (i < orig.length) {
            ...
            switch (true) {
                case c < 128:
                    out.push(String.xfromCharCode(c));
                    break;
                case c < 2048:
                    out.push(String.xfromCharCode(192 + (c >> 6)));
                    out.push(String.xfromCharCode(128 + (c & 63)));
                    break;
                case c >= 55296 && c < 57344:
                    c -= 55296;
                    d = orig.charCodeAt(i++) - 56320 + (c << 10);
                    out.push(String.xfromCharCode(240 + ((d >> 18) & 7)));
                    out.push(String.xfromCharCode(144 + ((d >> 12) & 63)));
                    out.push(String.xfromCharCode(128 + ((d >> 6) & 63)));
                    out.push(String.xfromCharCode(128 + (d & 63)));
                    break;
                default:
                    out.push(String.xfromCharCode(224 + (c >> 12)));
                    out.push(String.xfromCharCode(128 + ((c >> 6) & 63)));
                    out.push(String.xfromCharCode(128 + (c & 63)));
            }
        }
        return out.join("");
...
// matches <foo>...</foo> extracts content
    ...
    var __utf16le = function (b, s, e) {
        var ss = [];
        for (var i = s; i < e; i += 2) ss.push(String.xfromCharCode(__readUInt16LE(b, i)));
    ...
    var __utf8 = function (b, s, e) {
        var ss = [];
        for (var i = s; i < e; i++) ss.push(String.xfromCharCode(__readUInt8(b, i)));
        return ss.join("");
    };
    var ___utf8 = __utf8;
        ...
        switch (t) {
            case 'dbcs':
                ...
                if (has_buf && Buffer.isBuffer(this)) o = this.slice(this.l, this.l + 2 * size).toString("utf16le");
                    ...
                    o += String.xfromCharCode(__readUInt16LE(this, loc));
                    loc += 2;
                }
                size *= 2;
                break;
    ...
    function encode_col(col) {
        var s = "";
        for (++col; col; col = Math.floor((col - 1) / 26)) s = String.xfromCharCode(((col - 1) % 26) + 65) + s;
        return s;
    }
    ...
    var DBF = (function () {
        ...
        function dbf_to_aoa(buf, opts) {
            ...
            while (ft == 0x02 ? d.l < d.length && d[d.l] != 0x0d : d.l < hend) {
                ...
                d.l += ww;
                field.type = String.xfromCharCode(d.read_shift(1));
        ...
        for (var ff = 0, fflen = formula[0].length; ff < fflen; ++ff) {
            ...
            switch (f[0]) {
                ...
                case 'PtgRef3d': /* [MS-XLS] 2.5.198.85 */
                    ...
                    var w = sname;
                    /* IE9 fails on defined names */ // eslint-disable-line no-unused-vars
...
// 2.3.2
                ...
                switch (Rn) {
                    ...
                    default:
                        switch (R.n) { /* nested */
                            ...
                            case 'DSF':
                                break;
                            case 'BuiltInFnGroupCount': /* 2.4.30 0x0E or 0x10 but excel 2011 generates 0x11? */
                                break;
                            /* View Stuff */
                            case 'Window1':
                            case 'HideObj':
    ...
    function write_binary_type(out, opts) {
        switch (opts.type) {
            ...
            case "binary":
                var bstr = "";
                // $FlowIgnore
                for (var i = 0; i < out.length; ++i) bstr += String.xfromCharCode(out[i]);
    ...
    function sheet_to_txt(sheet, opts) {
        ...
        return String.xfromCharCode(255) + String.xfromCharCode(254) + o;
    }
